...
while (inp := inpf.readline().strip()): # Runs until the pointer reaches the end
    if inp[pointer] == "#": # Checks if the toboggan encounters a tree
        trees += 1
    pointer = (pointer + 3) % len(inp)
print(trees)
...
def check_trees(right, down=1):
    inpf.seek(0) # Resets the pointer
    trees = 0
    pointer = 0
    inp = inpf.readline().strip() # Reads one line
    while inp: # Runs until the pointer reaches the end
        if inp[pointer] == "#": # Checks if the toboggan encounters a tree
            trees += 1
        pointer = (pointer + right) % len(inp) # Moves to the right by {right} spaces
        inp = [inpf.readline().strip() for i in range(down)][-1] # Reads {down} lines
    return trees
